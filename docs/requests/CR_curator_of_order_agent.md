...
Introduce a new agent role: **Curator of Order** — a repository librarian and refactoring scalpel that keeps agents/skills/docs/decisions/MCP tooling organized. Also add minimal repo structure (indexes + templates) to scale to more agents and capabilities while preserving compatibility with the current agent contract and skills.
