...
We need a deterministic-ish, configurable procedural generator that can create nested star systems (stars → planets → moons, plus belts/specials) suitable for interactive visualization and editing.

## Goals / non-goals

- **Goals**:
  - Generate plausible hierarchical systems with tunable density/structure.
  - Keep the generator modular (topology presets / post-processing phases).
  - Support “special” features (belts, comets, disks, nebulae, rogues, black holes, etc.).
- **Non-goals**:
  - Physical realism (this is a visualization-first generator).
  - Perfectly stable outputs across major algorithm rewrites.

## Key concepts / terminology

- **Topology preset**: an L-system grammar + parameters that shape hierarchy.
- **Post-processing**: phases that enrich the topology with additional features.
- **Determinism**: optional seeded generation (see PRNG docs) for reproducibility.

## System overview

This document is the “deep dive” spec: architecture, parameters, and the phase-by-phase generation approach.
...
- The generator outputs structures compatible with app state/types (stars/bodies, optional groups, and optional “specials” like belts/disks/nebulae).
- Core invariants:
  - Hierarchies are acyclic.
  - Parent/child relationships are consistent (child references exist).
  - IDs are unique within a generation run.

## Key flows

- End-to-end flow diagram: `docs/design/ALGORITHM_FLOW.md`
- “Generate from config” is the primary flow; presets primarily affect the topology phase.

## Tradeoffs / alternatives

- **Visualization-first**: prioritizes interesting, editable systems over astrophysical realism.
- **Preset-based topology**: makes it easy to get variety, at the cost of more moving parts (grammar + post-processing).

## Open questions

- Should we treat some “specials” as a separate post-processing plugin interface (to make feature toggles more orthogonal)?
