...
- Developers who want **reproducible procedural generation** (same seed → same universe).

## Prerequisites

- Comfortable reading TypeScript.
- Able to run Node locally for the demo (`node ...`).
...
const rng = createPRNG('solar-system-alpha');
const float = rng.float();      // [0, 1)
const int = rng.int(1, 100);    // [1, 100] inclusive
const bool = rng.bool(0.7);     // true with 70% probability
```
...
- **“Same seed but different result”**: confirm you are not mixing `Math.random()` with the PRNG path (the generator should be fully seeded/deterministic).
- **“Different results across environments”**: ensure everything uses 32-bit integer math (no reliance on platform-specific floating behavior).
- **“Need independent randomness for subsystems”**: use `rng.fork('label')` so each subsystem has a stable, independent stream.
