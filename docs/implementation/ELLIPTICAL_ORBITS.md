...
- Existing circular orbits continue to work; elliptical parameters default to circular behavior when unset.
- Orbit editing UI should allow switching between “simple” and “advanced/elliptical” modes without losing data unexpectedly.

## Data model changes

- **Types**: orbit parameters expanded beyond `orbitalDistance` to include ellipse shape/offset/orientation.
- **State**: bodies store the additional orbit fields alongside existing orbit fields.
- **Persistence**: saved universes remain backward compatible (missing fields treated as defaults).
...
- UI:
  - Orbit editing UI (panel/editor) for advanced parameters
- State:
  - Body orbit fields stored in the main universe state
- Infra:
  - (None)
- Docs:
  - This doc + related orbit/rogue documentation

## Compatibility / migrations

- Backward compatible: existing circular orbits load as elliptical with `eccentricity = 0` and zero offsets/rotations.

## Verification

- Manual checks:
  - Create/edit a body with `eccentricity > 0` and confirm the visual orbit updates.
  - Set orbit plane rotation and confirm the orbit tilts in 3D.
  - Confirm older saved universes without ellipse fields still load and simulate.
- Scripts:
  - `npm run typecheck`
  - `npm run build`
...
- Consider adding a small “orbit parameter validation” section in the UI to prevent obviously invalid values.
