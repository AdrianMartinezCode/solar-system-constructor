...
- Rings can be generated deterministically and edited per-planet in the UI.
- Rings render as lightweight meshes that move with the host planet.
...
- Rings are generated deterministically (seeded) and attached to planet bodies.
- Rendering uses lightweight ring meshes that inherit the host planet’s transforms.

## Files touched

- UI:
  - Planet/body editor ring controls
  - Ring rendering components
- State:
  - Planet/star data model includes optional ring metadata
- Docs:
  - This doc

## Performance considerations

- Keep ring rendering lightweight (low-geometry meshes, avoid per-frame heavy computation).

## Compatibility / migrations

- Backward compatible: `ring` is optional and absent for older saves.

## Verification

- Manual checks:
  - Generate a system with rings enabled and confirm rings render and follow planets.
  - Edit ring parameters in the editor and confirm visuals update.
- Scripts:
  - `npm run typecheck`
  - `npm run build`
...
- Consider supporting ringed moons if desired (data model already allows it).
