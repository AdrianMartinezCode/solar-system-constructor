...
- Rogues move independently of star systems; trajectory visualization can be toggled for clarity.
- Curved paths reuse the ellipse math path (similar parameter set to orbits).

## Data model changes

- **Types**: adds `RoguePlanetMeta` for motion/trajectory parameters.
- **State**: rogues exist as bodies with additional meta; motion updates are applied during tick/render.
- **Persistence**: fields are optional for backward compatibility.
...
- UI:
  - Rogue planet editor section(s) and trajectory visualization controls
- State:
  - `src/types.ts` (`RoguePlanetMeta`)
  - Motion/trajectory update logic (tick/render)
- Docs:
  - This doc

## Performance considerations

- Trajectory visualization should be downsampled/bounded for large universes to avoid excessive line geometry.

## Compatibility / migrations

- Backward compatible: when `pathCurvature` is unset/0, rogues use linear drift.

## Verification

- Manual checks:
  - Generate rogues with linear motion and confirm drift over time.
  - Enable curved trajectories and confirm looping paths + visualization.
- Scripts:
  - `npm run typecheck`
  - `npm run build`
...
- Consider adding presets for common rogue behaviors (fast flyby, slow drifter, wide loop).
