...
- Kuiper belts render as outer, icy “small body fields” and are controlled via generator settings.
- Kuiper belts share UI + rendering infrastructure with asteroid belts, but preserve distinct appearance/placement.

## Data model changes

- **Types**: uses `SmallBodyField` with `beltType: 'kuiper'` (+ icy visual parameters).
- **State**: fields are included in the universe snapshot and aggregate stats.
- **Persistence**: persisted with the universe.
...
**Updated files:**
- ✅ `docs/design/PROCEDURAL_GENERATOR.md` (Kuiper belt section added)
- ✅ `docs/guides/GENERATOR_QUICKREF.md` (Kuiper parameters table)
- ✅ `docs/UI_IMPLEMENTATION_SUMMARY.md` (Kuiper UI controls)
- ✅ `docs/design/UI_REDESIGN_WINDOWED.md` (asteroid count clarification)
...
8. `docs/design/PROCEDURAL_GENERATOR.md` - Kuiper belt algorithm section
9. `docs/guides/GENERATOR_QUICKREF.md` - Kuiper parameters table
...
13. `docs/design/UI_REDESIGN_WINDOWED.md` - Asteroid count clarification
...
- UI:
  - Small body field rendering component(s)
  - Generator panel controls for Kuiper belts
- State:
  - Universe state fields for small body belts/fields
- Docs:
  - This doc + belt particle-field summary

## Performance considerations

- GPU particle field approach avoids per-object CPU overhead; keep particle counts bounded via detail controls.

## Compatibility / migrations

- Backward compatible: missing kuiper-belt fields/config default to “disabled”.

## Verification

- Manual checks:
  - Generate a universe with Kuiper belts enabled and confirm outer belt placement/appearance.
  - Toggle `smallBodyDetail` and confirm particle counts change.
- Scripts:
  - `npm run typecheck`
  - `npm run build`
...
- Consider unifying belt+kuiper docs further now that both are “small body fields”.
