...
- Belts appear as visual small-body fields and are controllable via generator settings (density/detail) and UI toggles.
- Belts move with their host system and are included/excluded via feature flags.

## Data model changes

- **Types**: introduces/uses a `SmallBodyField` representation for belts (visual-only).
- **State**: belts/fields live alongside the main universe state and are included in exports/stats.
- **Persistence**: persisted as part of the universe snapshot.
...
9. `docs/design/PROCEDURAL_GENERATOR.md` - Algorithm documentation
10. `docs/guides/GENERATOR_QUICKREF.md` - Quick reference
...
- UI:
  - Belt rendering component(s) (e.g. particle field object)
  - Generator panel controls (small body belts)
- State:
  - Universe state fields for belts/fields
- Docs:
  - This doc + belt particle-field summary

## Performance considerations

- Belt particle fields are GPU-driven; avoid per-particle CPU updates.
- Keep particle counts bounded via detail/density controls.

## Compatibility / migrations

- Backward compatible: older universes without belt fields behave as “no belts”.

## Verification

- Manual checks:
  - Generate a system with belts enabled and confirm belt visuals and expected counts.
  - Toggle belt-related settings and confirm particle counts/appearance adjust.
- Scripts:
  - `npm run typecheck`
  - `npm run build`
...
- Consider consolidating belt/kuiper docs around the unified “Small Body Fields” model.
