...
- Generation produces a fresh universe/system snapshot that the UI can load into state.
- When seeded, generation can be repeated to recreate the same universe (useful for sharing/debugging).

## Data model changes

- **Types**: generator outputs include stars/bodies (and optional groups/belts/specials depending on config).
- **State**: universe is stored in the main system store/state snapshot.
- **Persistence**: generated universes can be persisted via the existing save/load flow (if enabled in the app).

## Algorithms / approach

See “Files touched” below for where the core generator + examples live.
...
### 3. Documentation (`docs/design/PROCEDURAL_GENERATOR.md`)
**Size**: ~500 lines  
...
### 4. Quick Reference (`docs/guides/GENERATOR_QUICKREF.md`)
**Size**: ~350 lines  
...
- No migration required for existing saved universes unless generator output shape changes significantly.
- If generator configuration schema changes, treat it as additive where possible (preserve defaults).

## Verification

- Manual checks:
  - Generate multiple systems with and without seeds and confirm expected variety/reproducibility.
  - Enable grouping and confirm groups are created and stable.
- Scripts:
  - `npm run typecheck`
  - `npm run build`
...
- Consider adding a small set of “golden seeds” to detect accidental large behavior shifts if determinism becomes important.
