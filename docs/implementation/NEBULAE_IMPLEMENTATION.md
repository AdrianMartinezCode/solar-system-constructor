...
- Nebulae are large background/scene-composition elements; they do not affect physics.
- Visibility/association rules may tie nebulae to groups for isolation/filtering behavior.

## Data model changes

- **Types**: adds a `NebulaRegion` (visual-only) entity with geometry + visual parameters.
- **State**: stored alongside other universe-scale constructs (groups, fields, etc.).
- **Persistence**: persisted as part of the universe snapshot; seeded for reproducibility.
...
- UI:
  - Nebula rendering components/materials
  - Any nebula editor/visibility controls (if present)
- State:
  - `src/types.ts` (nebula types)
  - Generator integration for deterministic placement
- Docs:
  - This doc

## Performance considerations

- Nebula rendering can be fill-rate heavy; keep shader complexity and overdraw in check.

## Compatibility / migrations

- Backward compatible: nebula entities are additive; older saves simply have none.

## Verification

- Manual checks:
  - Generate a universe with nebulae enabled and confirm large regions render and look stable.
  - Isolate a group (if supported) and confirm associated nebulae behave as expected.
- Scripts:
  - `npm run typecheck`
  - `npm run build`
...
- Consider adding LOD/quality settings for nebulae if performance becomes an issue on low-end GPUs.
