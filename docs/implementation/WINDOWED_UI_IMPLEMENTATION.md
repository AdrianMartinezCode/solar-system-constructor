...
- Users can open multiple windows (Overview, Hierarchy, Generator, Stats, Editor, etc.) and arrange them freely.
- Windows support focus, minimize/restore, docking, resizing, and keyboard shortcuts (where applicable).

## Data model changes

- **Types**: window descriptors include position/size/z-index/docked/minimized state.
- **State**: `windowStore` manages window lifecycle + workspace presets.
- **Persistence**: custom layouts/presets can be saved/loaded.

## Algorithms / approach

See “Completed Components” and “Key Features” below for the implementation approach and feature breakdown.

## Files touched

- UI:
  - Window shell components + taskbar + header
- State:
  - `src/state/windowStore.ts`
- Docs:
  - This doc + windowed UI design/shortcut references
...
- Manual checks:
  - Open/close/focus/minimize/restore multiple windows.
  - Dock/undock windows and confirm layout updates correctly.
  - Switch workspace presets and confirm expected window layouts.
- Scripts:
  - `npm run typecheck`
  - `npm run build`
