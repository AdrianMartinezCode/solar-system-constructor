...
The prior stats view was too limited for diagnosing performance issues, validating generator output, and understanding universe composition at scale.

## Scope

- **In**:
  - New stats computation layer + telemetry
  - New chart components and a full tabbed Stats panel
  - Export paths (copy/JSON/CSV)
- **Out**:
  - Unrelated UI refactors outside the Stats window/pipeline
...
- Risk: stats computations and charts can add overhead if they run when the window is hidden.
- Rollback: disable/guard telemetry + heavy computation behind visibility checks, or revert to the prior minimal stats window.

## Verification

- Manual:
  - Open Stats, switch contexts (Global/Isolated/Selected), and verify counts change as expected.
  - Confirm exports (copy/JSON/CSV) and that charts render for small and large universes.
- Project-level:
  - `npm run typecheck`
  - `npm run build`
...
- Add lightweight automated “stats snapshot” validation if/when a test runner is introduced.