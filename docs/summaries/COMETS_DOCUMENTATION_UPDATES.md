...
Documentation was updated to include the Comets feature across the generator docs, implementation docs, and summaries.

## Motivation (why)

Keep technical documentation aligned with feature reality (comets are now implemented and should be discoverable from the main generator + feature docs).

## Scope

- **In**: doc updates for comets across generator + implementation documentation
- **Out**: application code changes (feature implementation itself)
...
- This doc is the change record; the “Files updated” list above describes the intended documentation touchpoints.

## Risk / rollback notes

- Risk: stale/incorrect docs if the feature evolves without updating references.
- Rollback: revert doc claims to match the last known shipped behavior.

## Verification

- Manually confirm that key “entry point” docs mention comets (generator docs, implementation doc, and UI summary) and that links resolve after taxonomy migration.

## Follow-ups

- As docs are migrated into taxonomy folders, update any remaining references to old `docs/*.md` root paths.
