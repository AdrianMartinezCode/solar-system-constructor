...
Provide an in-app UI for configuring generation (density, depth, presets, seed, features) so users can quickly iterate on interesting universes without hand-authoring bodies.

## Scope

- **In**:
  - Generator UI panel + presets + styling
  - Bridge layer from UI config → internal generator config
  - Integration example(s) and documentation
- **Out**:
  - Procedural generator algorithm changes (beyond the UI/bridge wiring)
...
- Risk: UI-generated “replace universe” flows can be destructive if not clearly communicated (reset/clear actions).
- Rollback: disable/hide the generator panel entrypoint while keeping generator APIs available.

## Verification

- Manual:
  - Open the Generator panel, try presets and custom configs, generate multiple times, and confirm the universe updates and persists.
- Project-level:
  - `npm run typecheck`
  - `npm run build`
...
- Consider wiring a “confirm before overwrite universe” guard if users frequently lose work.