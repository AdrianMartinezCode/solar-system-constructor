...
description: Execute one approved task prompt as a small diff, respecting file allowlists and verification commands.
---

# Dev Task Executor (TASK → diff → verify)
...
- When you are executing an approved task prompt under `docs/prompts/<slug>/task_<n>/...`.
- When you need strict scope discipline (file allowlist, one task per diff, verification per task).
